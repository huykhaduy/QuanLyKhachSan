...
public class DanhSachUuDaiVip implements ConsoleIO, ChucNangDS,Cloneable {
    private MyArray<ChiTietDichVu> dsUuDai = new MyArray<ChiTietDichVu>();

    public DanhSachUuDaiVip(){
    }

    public DanhSachUuDaiVip clone() throws CloneNotSupportedException {
        return (DanhSachUuDaiVip) super.clone();
    }

    @Override
    public void nhapThongTin() {
        while (true){
            System.out.println(Text.center("DANH SÁCH ƯU ĐÃI DÀNH CHO PHÒNG VIP",40,'-'));
            ...
            System.out.println("|"+Text.leftAt(10,Text.setLength("5. Thoát",27),' ')+"|");
    ...
    public void xuatThongTin() {
        if (dsUuDai.getLength()>0){
            System.out.println(DanhSachUuDaiVip.title());
            for (int i=0;i<dsUuDai.getLength();i++){
                System.out.println(dsUuDai.getAt(i).toString());
            }
        }
        else {
